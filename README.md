...
GitHub Actions で毎日3回（7:30 / 12:00 / 17:50 JST）最新ニュースを自分のLINEに届けるボット。  
**完全無料枠**で運用できます。

## アーキテクチャ

```
cron-job.org（外部スケジューラ・時刻に正確）
  → GitHub API (workflow_dispatch) でワークフロー起動
  → GitHub Actions で news_bot.py を実行
      → RSS取得（NHK / ITmedia / はてブ / GIGAZINE / Google News など）
      → Groq (Llama 3.3 70B) で重要記事を選定 + 要約
      → LINE Messaging API で自分にpush配信
```

> **起動方式について:** GitHub Actions の `schedule`(cron) は無料publicリポで
> 時刻が保証されず大幅遅延・スキップが起きたため、起動を外部スケジューラ
> （cron-job.org）からの `workflow_dispatch` に変更しています
> （経緯: `docs/NG-docs/NG理由.txt`）。
...
配信時刻は **cron-job.org 側のジョブ設定**で変更する（現在 JST 07:30 / 12:00 / 17:50）。
GitHub 側のワークフローには時刻設定は持たせていない（`workflow_dispatch` のみ）。

cron-job.org の各ジョブは、以下の GitHub API を POST してワークフローを起動する:

```
POST https://api.github.com/repos/XIT-ksuke/news-line/actions/workflows/news_delivery.yml/dispatches
Header:  Authorization: Bearer <Fine-grained token（Actions: Read and write）>
         Accept: application/vnd.github+json
Body:    {"ref": "main"}
```

> **なぜ外部スケジューラか:** GitHub Actions の `schedule`(cron) は無料publicリポで
> 時刻保証がなく、実測で数時間の遅延・発火スキップが発生したため（詳細:
> `docs/NG-docs/NG理由.txt`）。`workflow_dispatch` 起動は通常1分以内に開始する。
